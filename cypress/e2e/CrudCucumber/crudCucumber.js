...
let cucumberData
    ...
    before(() => {
        cy.fixture('cucumberCrudData/cucumberJsonData').then((data) => { cucumberData = data.cucumberData; });
    });
...
When('Click on button Delete Article', () => {
    cy.get(createOrEditPageElements.deleteButton).click()
})

Then('Assert first article on the page after delete article', () => {
    cy.intercept('GET', 'https://conduit-api.bondaracademy.com/api/articles?limit=10&offset=0').as('waitArticle')
    cy.get(articlesPageElements.articleName).eq(0).should('be.visible');
    cy.get(articlesPageElements.articleName).eq(0).should('not.have.text', cucumberData.editArticleTitle)
    cy.wait('@waitArticle')
})
